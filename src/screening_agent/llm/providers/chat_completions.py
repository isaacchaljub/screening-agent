...
that's Groq only; real OpenAI uses the Responses API instead (`providers/openai.py`). This module
is named for the calling convention, not a vendor, so a future OpenAI-compatible Chat Completions
vendor belongs here too.

Structured output is a `response_format={"type": "json_schema", "json_schema": {...}}`
chat-completions param, built by hand here in **best-effort mode** rather than via the SDK's
`.parse(response_format=<pydantic model>)` convenience — that builds an OpenAI *strict*-mode
schema (every field required, `additionalProperties:false`), which 400s on Groq for any
`Optional[SomeEnum]` field (e.g. `ExtractedFields.language`): Groq's strict-mode validator rejects
the anyOf-with-null shape pydantic naturally produces there, where OpenAI's own backend tolerates
...
`registry.GROQ_EVAL_MODEL`) — not a free choice, the only ones that support it. A schema failure
(more likely than on strict-schema vendors, given best-effort mode's weaker guarantee) surfaces as
`SchemaError`, which per R5 retries the *same* model with the parse error appended, not a vendor
fallback.
...
def complete_structured(
    ...
    # Built by hand, not via `.parse()` — see this module's docstring for why (Groq's strict-mode
    # schema rejects an `Optional[SomeEnum]` field like `ExtractedFields.language`).
    response_format = {
        "type": "json_schema",
